//////////////////////////////////////////////////////////////////
//File Name   : 2021202057_adv_server.c         //
//Date      : 2023/05/03               //
...
void sig_history(int sig){
    ...
    if(numofReq<10){ //if numberof request less than 10
        for(int i= numofReq-1 ; i>-1; i--){
            ...
            printf("%d\t",i+1);
            printf("%s\t", client_list[i].ip); //ip
            printf("%d\t", client_list[i].pid); //process id
            printf("%d\t", client_list[i].port); //port num
            printf("%s\n", ctime(&(client_list[i].timestamp))); //time 
        } 
    }
    else{ //if more than 9
        int j=10;
        for(int i=numofReq-1 ; i>numofReq-11 ; i--){
            ...
            printf("%d\t", client_list[i].pid); //process id
            printf("%d\t", client_list[i].port); //port num
            printf("%s\n", ctime(&(client_list[i].timestamp))); j--; //time
        }
    }
     printf("===========================================================\n");
    }
...
int main(int argc, char* argv[]) {
    ...
    while (1)
    {
       ...
       // printf("Server IP addressmymymymy: %s\n", server_ip_str);
        ...
        if ((pid = fork()) == 0) { //fork and child process
            close(socket_fd);
            ...
            strcpy(client_list[numofReq - 1].ip, ip_str);
            client_list[numofReq - 1].pid = pid;
